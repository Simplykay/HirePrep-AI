...
const InterviewRoom: React.FC<{ user: any, onFinish?: (result: InterviewResult) => void }> = ({ user, onFinish }) => {
  ...
  const [voiceClarity, _setVoiceClarity] = useState(90);
        ...
        <div className="lg:col-span-8 flex flex-col space-y-8 min-h-[500px] lg:h-full">
          <div className="flex-grow bg-slate-900 rounded-[3.5rem] border border-slate-800/50 relative overflow-hidden flex flex-col items-center justify-center p-8 shadow-inner">
            ...
            <div className="absolute inset-0 flex items-center justify-center opacity-20">
              <div className={`w-48 h-48 md:w-96 md:h-96 rounded-full blur-[60px] md:blur-[100px] transition-all duration-1000 ${activeSpeaker === 'Interviewer' ? 'bg-blue-600 scale-110' : activeSpeaker === 'Candidate' ? 'bg-emerald-600 scale-125' : 'bg-slate-800 scale-100'}`}></div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-32 w-full max-w-3xl relative z-10">
              <div className="flex flex-col items-center space-y-4 md:space-y-6">
                <div className={`w-28 h-28 md:w-44 md:h-44 rounded-[2rem] md:rounded-[3rem] flex items-center justify-center transition-all duration-1000 border-4 transform ${activeSpeaker === 'Interviewer' ? 'bg-blue-600/10 border-blue-500 shadow-[0_0_60px_rgba(59,130,246,0.25)] scale-110' : 'bg-slate-950 border-slate-800/50 opacity-30'}`}>
              ...
              <div className="flex flex-col items-center space-y-4 md:space-y-6">
                <div className={`w-28 h-28 md:w-44 md:h-44 rounded-[2rem] md:rounded-[3rem] flex items-center justify-center transition-all duration-1000 border-4 transform ${activeSpeaker === 'Candidate' ? 'bg-emerald-600/10 border-emerald-500 shadow-[0_0_60px_rgba(16,185,129,0.25)] scale-110' : 'bg-slate-950 border-slate-800/50 opacity-30'}`}>
                ...
                <div className="flex items-end justify-between space-x-0.5 h-10 bg-slate-950/50 rounded-xl p-2 border border-slate-800/50">
                  {[...Array(30)].map((_, i) => {
                    ...
                    return (
                      <div
                        key={i}
                        className={`w-full rounded-sm transition-all duration-75 ${isActive
