...
const FeedbackReport: React.FC<FeedbackReportProps> = ({ data, role, onReset, recordingUrl, transcript }) => {
  ...
  const handleExport = () => {
    ...
    const filename = `HirePrep-Dossier-${(role || 'Candidate').replace(/\s+/g, '-')}-${new Date().toISOString().split('T')[0]}.pdf`;
    
    // Temporarily ensure transcript is visible for PDF
    const wasTranscriptVisible = showTranscript;
    ...
    setTimeout(() => {
        if ((window as any).html2pdf) {
            (window as any).html2pdf().set(opt).from(element).save().then(() => {
                ...
                setShowTranscript(wasTranscriptVisible);
            });
        } else {
            window.print();
            setIsExporting(false);
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-12 animate-in zoom-in-95 duration-700 pb-20" id="dossier-content">
      <div className="bg-slate-900 p-10 md:p-14 rounded-[40px] shadow-2xl border border-slate-800 relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between mb-16 space-y-8 md:space-y-0">
          <div className="text-center md:text-left">
            ...
            <p className="text-slate-500 font-bold uppercase tracking-widest text-[10px] mt-2">Dossier ID: HP-{Math.floor(Math.random() * 9000) + 1000} | Role: {role || 'General'}</p>
          </div>
        ...
        {recordingUrl && (
            <div className="mb-16 p-6 bg-slate-950 rounded-3xl border border-slate-800" data-html2canvas-ignore="true">
                <h3 className="text-[10px] font-black uppercase text-slate-500 tracking-[0.2em] mb-4 flex items-center">
                    <i className="fas fa-play-circle mr-2 text-emerald-500"></i> Session Recording
                </h3>
                <audio controls className="w-full rounded-lg" src={recordingUrl} controlsList="nodownload">
        ...
        {transcript && transcript.length > 0 && (
             <div className="p-10 bg-slate-950 rounded-[40px] border border-slate-800 mb-12">
                <div className="flex items-center justify-between mb-8 cursor-pointer" onClick={() => setShowTranscript(!showTranscript)} data-html2canvas-ignore="true">
                   <h3 className="font-black text-xs uppercase tracking-[0.2em] text-slate-500 flex items-center">
                        <i className="fas fa-align-left mr-4"></i> Session Transcript
                   </h3>
                   <i className={`fas fa-chevron-${showTranscript ? 'up' : 'down'} text-slate-500`}></i>
                </div>
                
                {/* For PDF Generation, we always render this if needed, but hide with CSS if collapsed in UI */}
                <div className={`${showTranscript ? 'block' : 'hidden'} space-y-4`}>
                    {transcript.map((t, i) => (
                        <div key={i} className={`flex flex-col ${t.role === 'Candidate' ? 'items-end' : 'items-start'}`}>
                            <span className="text-[9px] font-bold text-slate-500 uppercase mb-1">{t.role === 'Candidate' ? 'You' : 'Interviewer'}</span>
                            <div className={`p-4 rounded-2xl text-xs max-w-[80%] leading-relaxed ${
                                t.role === 'Candidate' 
                                ? 'bg-emerald-600/10 border border-emerald-500/20 text-slate-200' 
        ...
        <div className="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-6" data-html2canvas-ignore="true">
          <button 
            onClick={onReset}
          ...
          <button 
            ...
            className="flex-1 py-6 bg-emerald-600 text-white rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-emerald-500 transition-all flex items-center justify-center space-x-4 shadow-2xl shadow-emerald-900/40 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {isExporting ? <i className="fas fa-spinner fa-spin"></i> : <i className="fas fa-file-pdf"></i>}
            <span>{isExporting ? 'Generating PDF...' : 'Export Full Dossier'}</span>
          </button>
        </div>
      </div>
    </div>
